...
**Status:** Pipeline complete. Test suite green (68 tests across 7 files, all passing — verified in this session).
...
audit trail, and executed **idempotently** so a payment is never double-charged.
...
stopping-rule layer clamps unsafe proposals in code; idle / overdue / unsafe cases are deliberately
halted or escalated to a human. This yields three hard guarantees:

1. **Explainability** — every stage transition is recorded in an append-only audit trail with full
   agent reasoning and a machine-readable rule id.
2. **Safety** — six code-enforced stopping rules (R1–R6) can override any LLM proposal; the model
   is never the last word on money.
3. **Idempotency** — a retry can never fire twice for the same `(case, attempt, action)`; enforced
   by a database `UNIQUE` constraint, not a promise.
...
- **Diagnoses** the root cause of a failure (insufficient funds, card expired, mandate revoked,
  bank timeout, …) with a structured, confidence-scored output.
...
- **Acts** idempotently — a retry can never fire twice for the same `(case, attempt, action)` —
  and logs everything to an append-only audit trail.
...
**Note on documentation drift:** `README.md` and `PROGRESS.md` state "63 tests across 6 files."
The actual, current suite is **68 tests across 7 files** — the 5 tests in `tests/test_metrics.py`
were added after those files were last updated. The count and file inventory below reflect the
verified, current state of the repository.
...
| Persistence / ORM | SQLAlchemy 2 (SQLite by default; Postgres-compatible schema) |
| Async job queue | Celery + Redis / **Upstash** (`rediss://` over TLS); eager mode for tests/demos |
| LLM backend | OpenAI-compatible client (`openai` SDK) → local **Ollama** (e.g. `qwen2.5:32b-instruct`); deterministic offline shim |
| Outbound HTTP (Razorpay live) | `httpx` |
| Templating (dashboard renders) | Jinja2 (declared; dashboard currently hand-builds HTML) |
| Testing / lint / typing | pytest, Ruff, mypy (strict) |
...
- **SQLite** (default dev engine, `sqlite:///reclaim.db`, plus per-run `reclaim_fresh_*.db` files
  for the batch demo). Schema is intentionally **PostgreSQL-compatible**: `JSON` for nested
  payloads, `String` for enums, indexed unique constraints for dedupe + idempotency.
...
│       ├── stopping_rules.py        # code-enforced R1–R6 ("LLM proposes, code disposes")
...
└── tests/
    ...
    ├── test_state_machine.py        # 10 tests — transition-table legality
    ├── test_stopping_rules.py       # 13 tests — every R1–R6 override
...
# PROJECT_MASTER.md — this document (generated)
```

### Directory & key file roles
- **`src/reclaim/`** — the entire application, packaged with `setuptools` under
  `[tool.setuptools.packages.find] where = ["src"]`. This is a **src-layout** package.
- **`tests/`** — the full pytest suite (7 files, 68 tests), hermetic by construction.
- **Root config/docs** — `pyproject.toml` (build + tooling), `.env.example` (env shape),
  `README.md`, `PROGRESS.md`, `LICENSE`, `.gitignore`.
...
- **Class `Settings(BaseSettings)`** — `SettingsConfigDict(env_file=".env", ...)` with
  ...
  - **Razorpay block:** `razorpay_webhook_secret` (default `""`, **validator requires non-empty**),
    ...
    (`https://api.razorpay.com/v1`), `razorpay_retry_path` (default `""` — see below),
    `act_mode` (`Literal["stub","live"]`, default `stub`).
...
- **`AuditLogEntry`** — one immutable row in the append-only trail: `case_id`, `stage`
  (`"ingest"`, `"diagnose"`, `"decide"`, `"act"`…), `agent_reasoning`, `input_state` (dict),
  `decision`, `action_taken`, `outcome`, `fallback_triggered` (bool), `timestamp`.
  - `fallback_triggered` means **one** thing: the LLM call itself failed (not a rule override).
...
- **`CaseStateMachine`** — carries a case through its lifecycle, firing the listener on each move.
  ...
  - Lifecycle helpers (each calls `_move`): `ingest()` (trigger
    ...
    (`decision.stop`, via_stop=True, requires current==DECIDED), `resolve()`
    (`act.succeeded`, from ACTING), `escalate()` (`act.escalated`), `fail()` (`act.failed`).
  - `_move(target, *, trigger, via_stop=False, action=None)` — validates, rejects moving out of a
    terminal state, updates `self.current`, fires the listener, logs `STATE_TRANSITION`.
...
### 4.5 `src/reclaim/stopping_rules.py` — "LLM proposes, code disposes"
- **Responsibility:** Deterministic, **pure** (no I/O, no DB), post-hoc validation layer,
  independent of the LLM prompt. First matching rule wins.
...
- **`enforce(input_, proposed, settings, *, payment_method_update_count=0)`** — the six rules:

  | Rule | Condition | Enforced action |
  |------|-----------|-----------------|
  | **R1** | cause == `mandate_revoked` (retries disallowed) | `escalate_human` |
  | **R2** | `amount > escalation_amount_threshold` | `escalate_human` |
  | **R3** | `days_since_last_attempt > escalation_days_threshold` | `escalate_human` |
  | **R4** | retry proposed and `attempt_number > max_retries_per_cycle` | `stop` |
  | **R5** | `request_payment_method_update` and `payment_method_update_count >= email_cap_per_7d` | `escalate_human` |
  | **R6** | `retry_now` proposed but cooldown not elapsed | `retry_scheduled` (never retry_now) |
  ...
  - If no rule fires, the proposal stands but its `scheduled_at` is validated/re-scheduled.
  - Every override sets `overridden=True` with a human-readable reason and machine-readable
    `rule` id so the audit trail and metrics can show a naive proposal being clamped in code.

---

### 4.6 `src/reclaim/llm_client.py` — LLM client (offline shim + online Ollama)
...
- **`FallbackResult[T]`** — `output: T`, `fallback_triggered: bool`.
- **`class LLMWrapper`** — convenience wrapper with fallback built in:
  - `diagnose(input_)` → returns `FallbackResult`; on any exception falls back to
...
### 4.8 `src/reclaim/db.py` — SQLAlchemy 2 persistence layer
...
- **`build_engine(database_url)`** — sets `check_same_thread=False` for SQLite (shared sessions
  across FastAPI threads / Celery workers in dev) and `pool_pre_ping=True`.
...
### 4.10 `src/reclaim/razorpay_client.py` — Razorpay test-mode client
- **Responsibility:** Stub or live Razorpay calls, always with idempotency keys.
...
- **`class RazorpayClient`**:
  ...
  - `retry_payment(*, case_id, subscription_id, amount, attempt_number)`:
    ...
    - **LIVE mode**: `httpx.post` to
      `{razorpay_base_url}{razorpay_retry_path}` with `auth=(key_id, key_secret)`, the
      idempotency-key header, and `{"subscription_id": subscription_id}` body; `raise_for_status`
      on error; returns `True` on success. (Body shape is intentionally deferred to the
      configured endpoint's contract — never guessed.)

---

### 4.11 `src/reclaim/email.py` — Email stub
...
### 4.12 `src/reclaim/dispatcher.py` — Webhook → pipeline handoff
...
### 4.13 `src/reclaim/celery_app.py` — Celery app + broker config
...
- **`build_app(settings=None)`**:
  ...
  - `conf.update`: JSON serializers, `timezone="UTC"`, `enable_utc=True`,
    ...
    `task_eager_propagates=True`, `task_acks_late=True`, `worker_prefetch_multiplier=1`, and TLS
    opts (`broker_use_ssl` / `redis_backend_use_ssl`) when `redis_url` is `rediss://`.
...
### 4.14 `src/reclaim/tasks.py` — Celery task entrypoints
...
### 4.15 `src/reclaim/audit.py` — Append-only audit writer
...
### 4.16 `src/reclaim/repo.py` — Read/update helpers
- **Responsibility:** Keeps the pipeline/Act/metrics layers from reaching into SQL directly.
...
### 4.17 `src/reclaim/pipeline.py` — Pipeline orchestrator
...
- **`@dataclass CaseOutcome`** — `case_id`, `terminal_state`, `cause`, `action`, `action_taken`,
  ...
  (R1–R6 overrode a valid proposal), `skipped` (default False).
- **`class ConcurrencyLimiter`** — bounded semaphore around a callable, tracking peak concurrency
  (`max_workers >= 1` required); `peak` is measurable for tests/demo. `run(fn, *args, **kwargs)`.
...
- **`run_case(case_id, *, settings=None, db=None)`** — drives one case end-to-end:
  ...
  2. **Ingest → Diagnose:** `machine.diagnose()`, persist `DIAGNOSED`, build `DiagnoseInput`,
     call `call_with_backoff(wrapper.diagnose)`, write audit (`fallback_triggered` = LLM failure).
  3. **Decide:** `machine.decide()`, persist `DECIDED`, build `DecideInput`, call
     `call_with_backoff(wrapper.decide)`, derive `email_count` via
     `repo.count_recent_payment_method_updates`, run `enforce(... priority)`, write audit with
     `outcome="DECIDED rule=<R#> [OVERRIDE]"` (`fallback_triggered` = LLM failure only).
...
### 4.18 `src/reclaim/metrics.py` — Batch metrics
...
### 4.19 `src/reclaim/synthetic.py` — Synthetic webhook batch generator
...
- **`generate_batch(*, n_valid=60, n_duplicates=6, n_rejections=7, seed=42,
  ...
  standard/silver/gold), payment history. Builds rejections: `malformed-json`, `missing-event-type`,
  `unmappable-amount`, `tampered-body` (flipped last char → signature mismatch), `missing-signature`,
  and random `empty-body` extras. Ends with a safety invariant `assert` on valid counts.
- **`render_delivery(w)`** — serializes a delivery exactly as the HTTP boundary sends it
  (`headers` + `body`).

---

### 4.20 `src/reclaim/batch.py` — Batch-run CLI entrypoint
...
### 4.21 `src/reclaim/api.py` — FastAPI application
- **Responsibility:** HTTP boundary: webhook ingestion + read-only audit surfacing.
- **Routes:** `GET /health`, `POST /webhook/razorpay`, `GET /cases/{case_id}`, `GET /dashboard`,
  `GET /metrics`.
...
- **`case_detail(case_id, fmt="html")`** — per-case decision trail; `?fmt=json` returns a
  ...
  `audit_trail`); default renders an HTML page.
...
- **`test_pipeline.py` (9 tests)** — end-to-end: healthy case recovers; mandate-revoked escalates
  ...
  limiter bounds peak.
- **`test_state_machine.py` (10 tests)** — full lifecycle legal; illegal edge raises; terminal
  states absorbing; DECIDED→RESOLVED requires `via_stop`; via_stop legal; decision flow stop
  resolves without acting; retry routes through ACTING; escalate/fail terminal; listener records
  transition metadata; decision flow requires DECIDED.
- **`test_stopping_rules.py` (13 tests)** — every R1–R6 rule for its exact forced override;
  attempt at/below max allowed; email cap under allowed; cooldown math; compliant proposal
  untouched; R1 precedence over R2; valid future `scheduled_at` kept; enforce returns valid
  Pydantic output.
...
|---------|-----------------|
| `CaseState` | `INGESTED`, `DIAGNOSED`, `DECIDED`, `ACTING`, `RESOLVED`, `ESCALATED`, `FAILED` (last three terminal) |
...
### 5.2 Relational schema (SQLAlchemy 2, Postgres-compatible)
**`recovery_cases`**
...
duplicate Act can never double-charge.
...
```
 [1] INGEST        [2] DIAGNOSE      [3] DECIDE         [4] ENFORCE          [5] ACT            [6] TERMINAL
  ...
  (X-Razorpay-      parse (Pydantic)  one bounded       R1-R6 dispose       idempotently
   Signature)       dedupe            action            (independent of     (stub or live)
   -> 401/422       (event_id UNIQUE)                   the prompt)
```
...
3. **Diagnose:** `pipeline.run_case` sends the case through `CaseStateMachine.diagnose()`
   (INGESTED → DIAGNOSED), persists it, builds a `DiagnoseInput`, and calls the LLM (offline shim or
   online Ollama via `LLMWrapper.diagnose`, with exponential backoff in online mode). The output
   `DiagnoseOutput(cause, confidence, reasoning)` is written to the audit trail.
...
5. **Enforce (the guard):** `stopping_rules.enforce` runs R1–R6 over the proposal with the
...
7. **Act (idempotent):** `act.execute_action` **claims the ledger first**
   ...
   - `request_payment_method_update` / `escalate_human` → `email.send_email_message` stub → RESOLVED
     / ESCALATED.
8. **Log:** each stage appends an immutable `AuditLogRow` (append-only), and the case row's `state`
   is updated at each transition. Terminal states are absorbing.
9. **Surfacing:** `GET /cases/{case_id}` (HTML or `?fmt=json`), `GET /dashboard` (HTML), and
   `GET /metrics` (JSON) let a reviewer read the full decision trail and batch metrics.
...
| `RAZORPAY_RETRY_PATH` | `""` | **Must be set for live** — never guessed |
...
### 7.6 Run the tests
```bash
pytest                 # 68 tests, all passing (verified)
```
...
1. Set `LLM_MODE=online` + your Ollama URL/model, `ACT_MODE=live` + Razorpay test keys + confirm
   `RAZORPAY_RETRY_PATH`, and wire a real provider into `email.send_email_message`.
...
- **Modules are single-purpose files** (one responsibility per module): `state_machine.py`,
  `stopping_rules.py`, `pipeline.py`, `act.py`, `webhook.py`, etc.
...
- **"LLM proposes, code disposes"** — the Decide agent's output is always a *proposal*; a pure,
  unit-tested, prompt-independent enforcement layer (`stopping_rules.py`) is authoritative.
- **Zero-halo** — required secrets fail loud at load/config time; never guess an API route or wire
  format (`RAZORPAY_RETRY_PATH` must be confirmed; `require_live_credentials`).
- **Idempotency as a DB constraint** — the `ExecutedActionRow` UNIQUE guard, not a promise.
...
- 68 tests across 7 files, covering every domain boundary: webhook signature/parse/dedupe,
  state-machine legality, all six stopping rules, Pydantic invariants, synthetic generator
  invariants, end-to-end fallback/idempotency/state flows, and metric-counter independence.

### 8.7 Documentation & repository conventions
- `README.md` — primary human-facing documentation, incl. the demo batch report and tech stack.
...
| Path | Role |
|------|------|
| `src/reclaim/` | Entire application (21 modules) |
| `tests/` | 68 tests across 7 files |
...
*Documented and verified in-session on 2026-08-31. Test suite: 68 passed. NOTE: `README.md` and
`PROGRESS.md` cite "63 tests / 6 files"; the verified current suite is 68 tests / 7 files
(including the 5 `test_metrics.py` tests).*
